...
export const savePet = async (req, res) => {
    try {
        ...
        const data = req.body //data trae todo 
...
export const getPets = async (req, res) => {
    ...
    try {
        ...
        const petWithOwnerNames = await Promise.all(pets.map(async (pet) =>{
            const owner = await User.findById(pet.keeper);//keeper es el id de la mascota
            return {
                ...pet.toObject(),
...
export const updatePet = async (req, res = response) => {  //Actualizar mascota
    try {
